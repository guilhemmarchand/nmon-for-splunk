...
# Modified by Guilhem Marchand 26102014: Improved APP dir definition (are we running nmon / TA-nmon-perl / PA-nmon)
...
if [ -d "$SPLUNK_HOME/etc/apps/nmon" ]; then
        ...
        echo "`date`, ERROR, the APP directory could not be defined, is nmon / TA-nmon-perl / PA-nmon installed ?"
        exit 1
...
# In first option, search for a local nmon.conf file located in $SPLUNK_HOME/etc/apps/nmon|TA-nmon-perl|PA-nmon/local

if [ -f $APP/local/nmon.conf ]; then
	. $APP/local/nmon.conf

# In second option, search for the main nmon.conf file located in $SPLUNK_HOME/etc/apps/nmon|TA-nmon-perl|PA-nmon/default
