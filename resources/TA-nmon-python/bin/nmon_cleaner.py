...
parser.add_argument('--approot', action='store', dest='APP',
                    help='Set a custom value for the Application root directory '
                         '(default are: nmon / TA-nmon-python / PA-nmon)')
...
if not APP:
    ...
    # APP Directories for standard nmon, TA-nmon-python, PA-nmon
    if is_windows:
    ...
    if is_windows:
        TA_NMON_APP = SPLUNK_HOME + '\\etc\\apps\\TA-nmon-python'
    else:
        TA_NMON_APP = SPLUNK_HOME + '/etc/apps/TA-nmon-python'
    ...
    if os.path.exists(NMON_APP):
        ...
        msg = 'The Application root directory could not be found, is nmon / TA-nmon-python / PA-nmon installed ? We tried: '\
