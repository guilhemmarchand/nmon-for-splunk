...
if [ -d "$SPLUNK_HOME/etc/apps/nmon" ]; then
        ...
        echo "`date`, ${HOST} ERROR, the APP directory could not be defined, is nmon / TA-nmon-python / PA-nmon installed ?"
        exit 1
...
# Search for a local nmon.conf file located in $SPLUNK_HOME/etc/apps/nmon|TA-nmon-python|PA-nmon/local
